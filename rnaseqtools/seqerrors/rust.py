...
def mk_rustcall_cbumi_cell_gene(busfolder, outfile, nmax, aggr: bool, t2g):
    s1 = f"{PREFIX} cargo run --release -- --output {outfile} cb-umi-cell-gene --nmax {nmax} --busfolder {busfolder} --t2g {t2g}"
    if aggr:
        s1 = f'{s1} --aggr'
    # print(s1)
    os.system(s1)
...
def rust_read_tso_results(samplename, DIRECTORY):
    # reading the precompuated error estimates in the directory
    results_df = pd.read_csv(f'{DIRECTORY}/{samplename}/tso.csv')
    df_error  = beta_binomial_error_estimates(results_df)
    df_error['samplename'] = samplename
    return df_error
